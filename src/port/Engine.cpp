...
GameEngine::GameEngine() : dictionary(nullptr) {
    ...
    loader->RegisterResourceFactory(std::make_shared<SM64::ResourceFactoryBinaryTextV0>(), RESOURCE_FORMAT_BINARY,
                                    "Text", static_cast<uint32_t>(SM64::ResourceType::Text), 0);
...
extern "C" void GameEngine_ReloadAllResources() {
    // Ship::Context::GetInstance()->GetResourceManager()->DirtyResources({ "shaders/*" }, { "assets/anims/*", "sound/*" });
